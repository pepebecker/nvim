* Tangle
:PROPERTIES:
:TANGLE: ~/.config/nvim/lua/pepe/custom/tangle.lua
:END: 

** Define a command to tangle the file
   Using treesitter, get all code blocks within sections that have a special
   *:TANGLE:* property and extract this code into a file when a command is typed.
